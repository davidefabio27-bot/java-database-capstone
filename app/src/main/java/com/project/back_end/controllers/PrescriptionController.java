...
public class PrescriptionController {
    ...
    public ResponseEntity<Map<String, String>> savePrescription(@PathVariable String token,
        ...
        Map<String, Object> validation = appService.validateToken(token, "doctor");
        if (validation.containsKey("message")) {
            response.put("message", "Invalid or expired token");
            return new ResponseEntity<>(response, HttpStatus.UNAUTHORIZED);
        }

        // Aggiorna lo status dell'appuntamento
boolean updated = appointmentService.changeStatus(1, prescription.getAppointmentId());
